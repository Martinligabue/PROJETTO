...
prelettura:		
	li $t1,0 		#si resettano $t0 e $t1, così che il ciclo di conta non interferisca
	move $t0, $s0
...
lettura:
	...
	li $t2, 0	 	#contatore che parte da 0 per il controllo a sx
...
salvalettera:
	li $t3,0 		#$t3 diventa una variabile numerica uguale a 0 (non e' più un indirizzo)
	sb $t1, ($s2) 		#si carica la lettera non doppia nello space
	addi $s2,$s2,1		#si aumenta lo space per passare alla prossima posizione
	move $t5,$s1 		#$t5 diventa il contatore che verrà utilizzato in controllo a dx per non modificare $s1 e poterlo riutilizzare
